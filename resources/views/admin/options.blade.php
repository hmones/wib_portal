...
@section('content')
    ...
    <div class="ui centered container">
        <h3 class="ui blue header"> <i class="stop wib bullet icon"></i> Registered organization types</h3>
        <div class="ui basic segment">
            <table class="ui celled stackable table">
                ...
                @forelse ($entityTypes as $entityType)
                    <tr id="entity_type_{{$entityType->id}}">
                        <td>
                            {{$entityType->name}}
                            <input type="hidden" name="entityType_{{$entityType->id}}_name" value="{{$entityType->name}}">
                        </td>
                        <td>
                            <i class="{{$entityType->icon}} icon"></i> ({{$entityType->icon}})
                            <input type="hidden" name="entityType_{{$entityType->id}}_icon" value="{{$entityType->icon}}">
                        </td>
                        <td class="center aligned">
                            <button class="ui blue basic button" onclick="handleUpdateEntityType('{{$entityType->id}}');">
                                <i class="pencil icon"></i>Edit
                            </button>
                            <button class="ui red basic button" onclick="handleDeleteEntityType('{{$entityType->id}}');">
                ...
                @empty
                    <tr>
                        <td colspan="4"><p>No Entity types registered currently!</p></td>
                    </tr>
                @endforelse
                    <tr class="ui inverted grey table form">
                        <td>
                            <input type="text" style="border-color:#d5d4d1;" name="entityType_name" id="" placeholder="Name of entity type">
                        </td>
                        <td class="ui field">
                            @include('partials.iconsDropdown', ['iconFieldName'=>'entityType_icon'])
                        </td>
                        <td>
                            <button class="ui blue basic fluid button" onclick="handleAddEntityType();"><i class="plus icon"></i>Create new</button>
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <form id="entityType_create_form" action="{{route('entityType.store')}}" method="POST">
                @csrf
    ...
    <br><br><br>
    <div class="ui centered container">
        <h3 class="ui blue header"> <i class="stop wib bullet icon"></i> Registered sectors</h3>
        <div class="ui basic segment">
            <table class="ui celled stackable table">
                <thead>
                <tr>
                ...
                @forelse ($sectors as $sector)
                    <tr>
                        ...
                        <td class="center aligned">
                            <button class="ui blue basic button" onclick="handleUpdateSector('{{$sector->id}}');"><i class="pencil icon"></i>Edit</button>
                            <button class="ui red basic button" onclick="handleDeleteSector('{{$sector->id}}');"><i class="trash icon"></i>Remove</button>
                        </td>

                    </tr>
                @empty
                    <tr>
                        <td colspan="4"><p>No sectors registered already!</p></td>
                    </tr>
                @endforelse
                    <tr class="ui inverted grey table form">
                        <td>
                            <input type="text" name="sector_name" id="" placeholder="Name of the sector" style="border-color:#d5d4d1;">
                        </td>
                        <td class="ui field">
                            @include('partials.iconsDropdown', ['iconFieldName'=>'sector_icon'])
                        </td>
                        <td>
                            <button class="ui blue basic fluid button" onclick="handleAddSector();"><i class="plus icon"></i>Create new</button>
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <form id="sector_create_form" action="{{route('sector.store')}}" method="POST">
                @csrf
...
@section('scripts')
    <script>
        ...
        function handleUpdateEntityType(entity_type_id){
            ...
            // $('input[name="entityType_update_icon"]').val(icon);
            $('.ui.dropdown.entityType_update_icon').dropdown('set selected', icon);
            $('#entityType_update_form').attr('action','/entityType/'+entity_type_id);
            $('.ui.small.modal.entityType').modal('show');
        }
        function handleDeleteEntityType(entity_type_id){
            $('#entityType_delete_form').attr('action', '/entityType/'+entity_type_id).submit();
        }
        ...
        function handleUpdateSector(sector_id){
            ...
            // $('input[name="sector_update_icon"]').val(icon);
            $('.ui.dropdown.sector_update_icon').dropdown('set selected', icon);
            $('#sector_update_form').attr('action','/sector/'+sector_id);
            $('.ui.small.modal.sector').modal('show');
        }
        function handleDeleteSector(sector_id){
            $('#sector_delete_form').attr('action', '/sector/'+sector_id).submit();
        }
    </script>
@endsection
