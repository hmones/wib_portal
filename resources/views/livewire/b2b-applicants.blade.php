    ...
    <div id="applicationsTable" style="display: {{$isOpen ? 'none' : 'block'}};">
        ...
        <table class="ui celled stackable table" aria-label="admin users table">
            ...
            @forelse ($applications as $application)
                <tr>
                    <td>
                        {{Str::of($application->user?->name ?? 'User deleted')->limit(50, $end='..')}}
                    </td>
                    <td>
                        <a href="mailto:{{$application->user?->email}}">{{$application->user?->email ?? 'User deleted'}}</a>
                    </td>
                    <td>
                        {{$application->entity?->name ?? 'Entity deleted'}}
                    </td>
                    <td>
